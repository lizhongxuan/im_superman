...
func (client *Channel) RunOnce(conn *net.TCPConn) {
	...
	go func() {
		...
		for {
			...
			if msg == nil {
				log.Info("client ",client.uid ," read end")
				return
			}
		}
	}()
...
func (client *Channel) HandleMessage(msg *Message) {
	log.Info("client ",client.uid," read msg:", msg)
	if msg == nil {
		client.wt <- msg
	...
	case MSG_AUTH_STATUS:
		...
		if msg.body.(*AuthenticationStatus).status == 0 {
			client.isOpen = true
		}
	case MSG_ACK:
...
func (client *Channel) HandlePing() {
...
func (client *Channel) Heart() {
	for  {
		...
		t:=time.Duration(rand.Int63n(1000)) * time.Millisecond + 30000* time.Millisecond
		time.Sleep(t)
		msg := &Message{cmd:MSG_PING}
		client.wt <- msg
	}
}